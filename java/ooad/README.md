...
##### 요구사항
```
- 좋은 요구사항은 시스템이 고객이 기대한 대로 동작해야 한다.
- 유스케이스는 시스템이 해야할 일을 자세하고 명확하게 설명한다.
- 하나의 유스케이스는 하나의 목표를 가지지만, 그 목표를 이루는 경로는 여러가지일 수 있다.
- 좋은 유스케이스는 시작조건, 종료조건, 외부구동자, 사용자의 명확한 가치를 가지고 있다.
- 유스케이스는 시스템에 추가되어야 할 불완전하거나 빠진 요구사항들을 분명하게 한다.
```
---

##### 분석과 설계
```
유스케이스는 고객들이 이해하기 쉬운 방식으로 작성하라.
각 유스케이스는 하나의 목표에만 집중해야 한다.
위임은 객체들을 다른 객체들의 구현상의 변화로부터 보호한다.
클래스들과 메소드들을 찾아내기 위해 유스케이스에서 명사와 동사를 분석하는 것을 본문 분석이라고 한다.
유스케이스의 명사들은 클래스 후보들이고, 동사들은 클레스의 메소드 후보들이다.
잘 디자인된 소프트웨어는 변경과 확장이 쉽다.
디자인이 유연하지 않으면, 변경하여야 한다.
각 클래스의 응집도를 높게 하라:클래스 각각은 하나의 일을 정말 잘하는 것에 중점을 두어야 한다.
소프트웨어 디자인을 진행하면서, 항상 높은 응집도를 위해 노력하여야 한다.
```
```
...
##### 변경
```
요구사항은 항상 변한다. 하지만 유스케이스들이 잘 만들어져 있으면, 새로운 요구사항에 맞추어 소프트웨어를 빠르게 바꿀 수 있다.
소프트웨어를 변경에 잘 견디도록 만드는 가장 쉬운 방법은 각 클래스가 변경의 이유를 하나만 갖도록 하는 것이다. 
시나리오는 유스케이스를 처음부터 끝까지 진행하는 하나의 경로를 의미한다.
```
##### 객체지향 원리
```
변화하는 것을 캡슐화하라
...
클래스는 행동과 기능에 관한 것이다.
클래스는 확장에는 열려있고, 수정에는 닫혀있어야 한다.(OCP) (유연성에 대한 것이고, 단순히 상속에 대한 내용은 아님)
...
##### 프로그래밍 기술
```
약정에 의한 프로그래밍은 소프트웨어 사용자가 지키기로 합의한, 소프트웨어가 어떻게 동작해야 하는지에 관한 계약을 설정하다.
방어적 프로그래밍은 다른 소프트웨어를 신뢰하지 않고, 다른 소프트웨어가 나쁘거나 안전하지 않은 정보를 제공하는지 확인하기 위해, 광범위하게 오류와 데이터를 점검한다.
```