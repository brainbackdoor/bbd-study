...
```
1. 프로듀서 API는 문자열 메시지, 토픽, 파티션, 브로커 목록 등을 호출
2. Key-Value 형식으로 데이터 객체를 생성
3. 데이터 객체를 Serialization
4. 토픽 파티션을 결정
5. 메타데이터를 가져올 브로컬 URL을 부트스트랩
6. 파티션 리더를 선정
7. 지원되는 API 버전을 가져와 결정
8. 카프카 브로커(리더)로 메시지 전송 -> 응답 수신 -> 메타데이터 반환 
```