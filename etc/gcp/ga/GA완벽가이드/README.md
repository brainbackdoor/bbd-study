...
```
분석 구현 현황을 가늠하고 더 특화된 추적과 보고의 필요성을 평가
...
```
최적화 프레임워크 
- 전략(로드맵 및 KPI 수립) -> 구현(중요한 것을 측정) -> 보고(구글 애널리틱스와 보조 도구) 
  -> 분석(실행 가능한 통찰력 도출) -> 최적화(정량적, 정성적 데이터를 기반으로 테스트 및 개인화) 
  -> 비즈니스 영향력(수익 증대, 비용절감 & 고객 만족도 개선)
```
```
분석의 현재 수준 진단하기
- 추적코드를 최신버전으로 사용하고 있는가?
- 여러 기기에서 접속하는 사용자 경로를 추적해야 하는가?
- 다른 도메인이나 하위 도메인, 타사 프로세서를 추적할 필요가 있는가?
- 데이터가 조직의 고객 분류체계(가입 여부 또는 회원 등급 등)와 콘텐츠 분류체계(카테고리 및 저자)를 제대로 반영하고 있는가?
- 개인 식별정보를 수집하지 않는 한도 내에서 개별 사용자의 추적이 필요한가?
- 데이터를 백엔드 시스템과 비교 검증해야 하는가?
- CRM 데이터와 통합이 필요한가?
- 애드워즈 등을 통해 리마케팅/리타깃팅 캠페인을 운영하는가?
- 다중 채널이 전환 행동에 미친 영향도를 측정하는가?
- 단일 보고서 제공이 가능한가?
```
```
프로세스와 의사소통의 어려움
...
가장 중요한 것을 측정하라.
철저한 진단이 핵심이다.
측정 계획은 할애한 시간과 노력만큼 좋아지게 되어 있다.
